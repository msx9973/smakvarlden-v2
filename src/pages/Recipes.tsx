                          ...
                          {item.ingredients.map((ing, ingIndex) => {
                            ...
                            return (
                              <div key={ingIndex} style={{ display:'grid', gridTemplateColumns:'1fr 86px 68px 92px 70px auto', gap:8, alignItems:'center' }}>
                                ...
                                <select className="inp" value={ing.unit || 'g'} onChange={e => updateIngredient(itemIndex, ingIndex, 'unit', e.target.value)} style={{ padding:'7px 8px', fontSize:12 }}>{['g','kg','ml','cl','l','st','flaska','burk','msk','tsk'].map(u => <option key={u}>{u}</option>)}</select>
                                ...
                                <select className="inp" value={matched?.unit ?? menuIngredientPriceUnit(ing)} disabled={Boolean(matched)} onChange={e => updateIngredient(itemIndex, ingIndex, 'priceUnit', e.target.value)} style={{ padding:'7px 8px', fontSize:12, background:matched ? 'var(--muted)' : 'var(--white)' }}>{['kg','liter','l','st'].map(u => <option key={u}>{u}</option>)}</select>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:8}}>
                {ings.map((x,i)=>(
                  <div key={i} style={{display:'grid',gridTemplateColumns:'1fr 90px 70px auto',gap:8,alignItems:'center',padding:'10px 12px',borderRadius:10,background:x.quantity?(x.matchedId?'var(--goldbg)':'var(--muted)'):'rgba(185,28,28,.05)',border:`1px solid ${x.quantity?'var(--border)':'rgba(185,28,28,.2)'}`}}>
                    ...
                    <select className="inp" value={x.unit} onChange={e=>updIng(i,'unit',e.target.value)} style={{padding:'6px 8px',fontSize:12}}>{['g','kg','ml','cl','liter','styck','flaska','burk','msk','tsk'].map(u=><option key={u}>{u}</option>)}</select>
